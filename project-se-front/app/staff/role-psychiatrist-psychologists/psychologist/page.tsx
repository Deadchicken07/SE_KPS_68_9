import { getPsychologistDashboard } from '@/lib/role-dashboard-api';

export const dynamic = 'force-dynamic';

export default async function PsychologistHomePage() {
  const {
    summaryCards,
    sessions,
    insights,
    ...
    therapistSchedule,
    recentResponses,
  } = await getPsychologistDashboard();
                ...
                <div className="mt-4 space-y-3">
                  <div className="rounded-2xl bg-white px-4 py-4 ring-1 ring-[#eadfd2]">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {summaryCards[2]?.value ?? '0'} ชุดคำตอบที่ต้อง review
                    </p>
                  </div>
                  <div className="rounded-2xl bg-white px-4 py-4 ring-1 ring-[#eadfd2]">
                    <p className="font-medium text-slate-900">เคส active ต่อเนื่อง</p>
                    <p className="mt-1 text-sm text-slate-500">
                      {summaryCards[3]?.value ?? '0'} consultation ที่ต้องอัปเดตแผน
                    </p>
                  </div>
                </div>
              </div>
          ...
          <article className="rounded-[34px] bg-white p-6 shadow-[0_20px_50px_rgba(88,63,43,0.06)] ring-1 ring-[#eadfd2] lg:p-7">
            ...
            <div className="mt-6 space-y-4">
              {insights.length ? (
                ...
                <p className="rounded-[24px] bg-[#fffaf6] p-5 text-sm text-slate-500 ring-1 ring-[#eadfd2]">
                  ยังไม่มี insight ใหม่จากแบบประเมินในตอนนี้
                </p>
              )}
            </div>
          </article>
            ...
            <article className="rounded-[34px] bg-[#4e2d1f] p-6 text-white shadow-[0_20px_55px_rgba(78,45,31,0.18)] lg:p-7">
              ...
              <div className="mt-6 space-y-4">
                {sessions.length ? (
                  ...
                  <p className="rounded-[24px] bg-white/8 p-5 text-sm text-white/70 ring-1 ring-white/10">
                    ยังไม่มี session ที่ต้องเริ่มในวันนี้
                  </p>
                )}
              </div>
            </article>
            ...
            <article className="rounded-[34px] bg-[#f4ecdf] p-6 ring-1 ring-[#ead7be] lg:p-7">
              ...
              <div className="mt-6 space-y-4">
                {activeConsultations.length ? (
                  ...
                  <p className="rounded-[24px] bg-white px-5 py-5 text-sm text-slate-500 ring-1 ring-[#eadfd2]">
                    ยังไม่มี consultation ที่ต้องติดตามต่อเนื่องในตอนนี้
                  </p>
                )}
              </div>
            </article>
          ...
          <article className="rounded-[34px] bg-white p-6 shadow-[0_20px_50px_rgba(88,63,43,0.06)] ring-1 ring-[#eadfd2] lg:p-7">
            ...
            <div className="mt-6 space-y-4">
              {therapistSchedule.length ? (
                ...
                <p className="rounded-[24px] bg-[#fffaf6] p-5 text-sm text-slate-500 ring-1 ring-[#eadfd2]">
                  ยังไม่มีข้อมูลตารางงานในช่วงนี้
                </p>
              )}
            </div>
          </article>
          ...
          <article className="rounded-[34px] bg-white p-6 shadow-[0_20px_50px_rgba(88,63,43,0.06)] ring-1 ring-[#eadfd2] lg:p-7">
            ...
            <div className="mt-6 space-y-4">
              {recentResponses.length ? (
                ...
                <p className="rounded-[24px] bg-[#fffaf6] p-5 text-sm text-slate-500 ring-1 ring-[#eadfd2]">
                  ยังไม่มีผลตอบแบบประเมินล่าสุดในตอนนี้
                </p>
              )}
            </div>
          </article>
