...
export class AppointmentsService {
  ...
  async getAllPaidAppointments(): Promise<any[]> {
    ...
    const appointments = await this.prisma.appointments.findMany({
      include: {
        users_appointments_user_idTousers: { select: { name: true, sur_name: true, user_id: true, title: true } },
        users_appointments_staff_idTousers: { 
          include: { 
            roles: true 
          } 
    ...
    return appointments.map(a => {
      ...
      if (range) {
        ...
        const hourlyRate = feeId && feeMap.has(feeId) ? feeMap.get(feeId)! : defaultRate;
        amount = durationHours * hourlyRate;
      }

      return {
        id: a.id,
        patientName: this.buildPatientName(a.users_appointments_user_idTousers?.title, a.users_appointments_user_idTousers?.name, a.users_appointments_user_idTousers?.sur_name, a.users_appointments_user_idTousers?.user_id || a.user_id || 0),
        staffName: this.buildConsultantName(a.users_appointments_staff_idTousers?.name, a.users_appointments_staff_idTousers?.sur_name),
  ...
  async rejectPayment(appointmentId: number) {
    return this.prisma.appointments.update({
      ...
      data: { 
        status: 'Not_paying',
        deposit_slip_file: null // Remove the reference to the image 
      } as any
    });
  }
